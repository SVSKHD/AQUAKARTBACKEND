...
const getBlogById = async (req, res) => {
  ...
  try {
    ...
    const relatedProduct = await AquaProduct.find({category:blogById.category})
    return res.status(200).json({ success: true, data: blogById, relatedProduct:relatedProduct });
  } catch (error) {
    return res
      .status(400)
